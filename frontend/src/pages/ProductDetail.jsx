	...
	return <div className="pdp">
				...
				<div className="main_product_info">
					...
					<div className="img_container" onMouseOver={() => setIsDisplayHovered(true)} onMouseOut={() => setIsDisplayHovered(false)}>  
						<img className={`main ${isDisplayHovered && "hidden"}`} src={currrentProduct.image} alt={currrentProduct.imageAlt} />
        				<img className={`contextImg ${isDisplayHovered && "show"}`} src={currrentProduct.contextualImageUrl} alt={currrentProduct.imageAlt} />		
					</div>

					{/* RIGHT */}
					<div className="product_info"> 
						<h2 className="h2 sb product_name">{currrentProduct.currrentProductname}</h2>
						...
						<div className="price_rating_container">
							<div className="price_container">
								...
								<span className="original_price tiny_text">${parseFloat((currrentProduct.price.currentPrice * 1.25).toFixed(2))} </span></div>)
								}
								<h2 className="h2 sb current_price">${currrentProduct.price.currentPrice}</h2>
							</div>
						...
						<div className="variation_container">
							<h5 className="h5 md">Variations  </h5>
								...
								<img src={productObj.image} alt={productObj.imageAlt}  className={variation === -1 && "shape_outline_active"}  onClick={()=> {setVariation(-1);setCurrentProduct(productObj)}}/>
								...
								{productObj.variants  && productObj.variants.map((variant, index)=>{ 
									return <img key={index} src={variant.image} alt={variant.imageAlt} className={variation === index && "shape_outline_active"} onClick={()=>{setVariation(index);setCurrentProduct(productObj.variants[index])}}/> 
								}) }
							</div>
						</div>
						<div className="action_container">
							<div className="stocks_container">
								<h5 className="h5 rg">Stock: <span className="md">{parseInt(currrentProduct.id.slice(-3)) } left</span></h5>
