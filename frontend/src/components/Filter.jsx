  ...
  return (
    <div className="accordion-item filter_item">
      ...
      <div id={title} className="accordion-collapse collapse show">
        <div className={`accordion-body h7 rg ${containerClass}`}>{children}</div>
      </div>
    </div>
  );
...
const Filter = () => {
  ...
  const handleFilterOn = () =>{
    ...
    const isPriceExpanded = document.getElementById('Price').classList.contains("show") 
    if(isPriceExpanded){  
      setShowLabel(true)  
    } 
  }
  ...
  const handleMaterialChange = (event) => {
    //add it to filter list
    setFilterList(filterList.includes("material") ? filterList : [...filterList, "material"]);
    setMaterial(event.target.value);
  };

  const handleColorChange = (event) => {
    //add it to filter list
    setFilterList(filterList.includes("color") ? filterList : [...filterList, "color"]);
  ...
  useEffect(() => {
	...
	return (
		<div className="filter">
			<button className="filter_button shape_outline h7 rg" onClick={handleFilterOn}>
      ...
      <div className={`overlay ${showFilter && "active"}`}> 
        ...
        <div className="filter_sidebar" >
          <h4 className="h4 sb">Filter <MdClose size={24} className="icon" onClick={handleFilterOff}/></h4>
          <div className="filter_content accordion accordion-flush" id="filterPirceAccordion">
            <FilterItem title="Price" buttonFunc={()=> setShowLabel(e => !e)} >
              <div className="price_range_sentence">From ${priceRange[0]} to ${priceRange[1] === 605 ? "\u221E" : "$" + priceRange[1]}</div>  
                <Slider
                  value={priceRange}
                  ...
                  valueLabelFormat={value => value === 605 ? "\u221E" : `$${value}`}
                  step={5}
                  min={0}
                  max={605}
            ...
            <FilterItem title="Material" containerClass="material_container"  > 
              ...
              <label className="h7 rg" id="material_particleboard">
                Particleboard
                <input
                  type="radio"
                  name="material"
            ...
            <FilterItem title="Color" containerClass="colors_container">
          ...
          <div className="button_container h7 md  ">
            <button className="white" onClick={handleClearAll}>Clear All</button>
            <button className="purple" onClick={handleApplyFilter}>Apply { filterList.length  > 0 && `(${filterList.length})`}</button>
          </div>
        </div>
      </div>
		</div>
